...
pub fn ideas_list(ideas: &[Idea]) -> Markup {
    html! {
        div class="h-full flex flex-row gap-4 p-4 overflow-auto divide-solid divide-slate-200 dark:divide-slate-700" {
            div #ideas_list class="h-fit max-h-full flex flex-col shrink basis-72 max-width-72 rounded-md bg-white dark:bg-slate-800 divide-y divide-solid divide-slate-200 dark:divide-slate-700 border-2 border-slate-200 dark:border-slate-700 overflow-auto" {
